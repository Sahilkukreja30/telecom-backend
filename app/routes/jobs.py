...
def export_jobs_csv(db=Depends(get_db)):
    ...
    headers = [
        "Job ID", "Worker", "Site ID", "Sector", "Circle", "Company", "Status", # <-- ADDED
    ...
    for job in cur:
        ...
        row = [
            ...
            job.get("siteId", ""),
            job.get("sector", ""),
            job.get("circle", ""),    # <-- ADDED
            job.get("company", ""),  # <-- ADDED
            job.get("status", ""),
            mac, rsn, az,
            job.get("createdAt", ""),
            job.get("updatedAt", ""),
...
def delete_job(job_id: str, db=Depends(get_db)):
    ...
    db.photos.delete_many({"jobId": {"$in": [job_id, str(_id)]}}) 
...
def export_xlsx_with_images(job_id: str, db=Depends(get_db)):
    # ... (This export is also fine, but you could add circle/company)
    try:
    ...
    photos = list(db.photos.find({"jobId": {"$in": [job_id, str(job_id)]}}).sort("_id", 1))
    ...
    headers = [
        "jobId", "workerPhone", "photoId", "type", "sector", "macId", "rsn", "azimuthDeg",
        ...
        try:
            ...
            with httpx.Client(timeout=20) as client:
                ...
                with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp:
                    ...
                    ws.add_image(xlimg, f"O{row_idx}")
